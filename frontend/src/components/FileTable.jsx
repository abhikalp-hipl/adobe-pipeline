              ...
              <tr key={`${file.name}-${idx}`} className="border-t align-top">
                ...
                <td className="p-2 border">
                  <div className="flex gap-2">
                    ...
                    <button type="button" title={canOpenJson ? "View Accessibility Report" : unavailableTitle} disabled={!canOpenJson} onClick={() => openFile("json", file?.outputs?.json_url, "Accessibility Report", { companionPdfUrl: file?.outputs?.pdf_url })} className="px-2 py-1 text-xs bg-gray-100 rounded hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed inline-flex items-center gap-1"><Code size={14} />Accessibility Report</button>
