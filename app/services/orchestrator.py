...
from app.services.precheck import validate_pdf_async
...
class Orchestrator:
    def __init__(self, db: AsyncSession, adobe_client: AdobeClient | None = None) -> None:
        self.db = db
        self.adobe_client = adobe_client or AdobeClient()
        logger.info("Orchestrator initialized")

    async def process_document(self, document_id: str, input_path: str | Path | None = None) -> dict[str, str]:
        ...
        try:
            await self._set_status(document, DocumentStatus.PROCESSING)
            ...
            if intake_path.suffix.lower() != ".pdf":
                ...
                pdf_path = await self.adobe_client.convert_to_pdf_async(
                    intake_path,
            ...
            precheck = await validate_pdf_async(pdf_path)
            ...
            generated_tagged_path, autotag_report_path = await self.adobe_client.auto_tag_async(
                pdf_path,
                output_dir=temp_path,
            ...
            await self._set_status(document, DocumentStatus.TAGGED)
            ...
            generated_report_path = await self.adobe_client.check_accessibility_async(
                tagged_output_path,
                report_dir=temp_path,
            )
            ...
            await self._set_status(document, DocumentStatus.CHECKED)
            ...
            # Step 3: completed.
            await self._set_status(document, DocumentStatus.COMPLETED)
    ...
    async def _get_document(self, document_id: str) -> Document:
        document = (await self.db.execute(select(Document).where(Document.id == document_id))).scalars().first()
        if not document:
